...
@property (nonatomic,strong) UILabel *newsTyptLabel;//类型
...
- (void)setUpView{
    ...
    //类型
    [self.contentView addSubview:self.newsTyptLabel];
}
...
-(UILabel *)newsTyptLabel{
    if(!_newsTyptLabel){
        _newsTyptLabel = [[UILabel alloc]init];
        [_newsTyptLabel setFrame:CGRectMake(self.newsImageView.current_x_w+self.newsTitleLabel.current_w/2+10.0, self.newsSourceLabel.current_y, self.newsSourceLabel.current_w,self.newsSourceLabel.current_h)];
        [_newsTyptLabel setTextAlignment:NSTextAlignmentRight];
        [_newsTyptLabel setTextColor:kAppGrayColor];
        [_newsTyptLabel setFont:UIFont_size(12.0)];
        
    }
    return _newsTyptLabel;
}
#pragma mark - setData
...
- (void)loadViewData{
    ...
    //类型
    [self.newsTyptLabel setText:_data.newsType];
}
- (void)awakeFromNib {
    [super awakeFromNib];
    // Initialization code
